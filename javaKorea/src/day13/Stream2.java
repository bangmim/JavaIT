...
//		int result = fc.makeFile("C:\\java");	//C 드라이브에 있는 java 폴더 내에 파일 생성
...
class FileCtrl{
	// 파일 만들기
	public int makeFile(String path) {	// 자료형(int)를 준 이유 : 생성에 실패하면 0, 성공하면 1을 반환
		int isSuccess = 0;
		...
		try {
			...
			isSuccess = 0;
			e.printStackTrace();	//콘솔창에 오류 내역을 적어준다
			return 0;
		} finally {
			sc.close();
			// try : 시도하고자하는 코드
