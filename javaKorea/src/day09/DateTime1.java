...
class MyTimer{
	...
	//SimpleDateFormat : 내가 원하는 형식대로 날짜와 시간을 재구성
	
	public void go() {
		Calendar c = Calendar.getInstance();	//getInstance(): 클래스 1회성 사용(값을 대입하거나 직접적으로 사용하는 것이 아닌 다른 곳에 전달해주기 위해 사용하는 것일 뿐)
		Date date = c.getTime();		//날짜와 시간이 ms단위로 저장됨
